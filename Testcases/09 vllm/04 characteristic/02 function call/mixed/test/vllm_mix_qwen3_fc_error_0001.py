...
class vllm_mix_qwen3_fc_error_0001(MindIEBenchMarkCase):
    """
    CaseNumber:
        vllm_mix_qwen3_fc_base_0001
    RunLevel:
    ...
    def procedure(self):
        ...
        assert  response["status"]==400, f'推理请求发送失败，失败原因为{response["status"]}'
        assert "Invalid value for `tool_choice`" in str(response), f'与预期不一致'

...
class vllm_mix_qwen3_fc_mode_no_think_0013(FunctionCallCaseBase):
    ...
    def procedure(self):
        ...
        self.assertTrue(message.get("tool_calls"), f"no_think 场景未返回 tool_calls: {response}")
        self.assertNotIn("reasoning_content", message, f"不应返回 reasoning_content: {message}")
